...
class MinioStorage:
    ...
    def __init__(self, logger=None):
        ...
        self.public_endpoint = minio_cfg.get('PUBLIC_ENDPOINT', minio_cfg['ENDPOINT'])
        self.logger = logger
    ...
    def _ensure_bucket(self):
        """Создаёт бакет и устанавливает публичную политику чтения."""
        try:
            ...
            if self.logger:
                self.logger.info(f"MinIO bucket '{self.bucket}' policy set to public-read")
        except S3Error as e:
            if self.logger:
    ...
    def upload_from_bytes(self, buf: bytes, object_name: str) -> str:
        ...
        Возвращает URL вида http://localhost:9000/plates/plate_cam1_42_1234.jpg
        """
        try:
            data = io.BytesIO(buf)
            ...
            url = f"http://{self.public_endpoint}/{self.bucket}/{object_name}"
            if self.logger:
